...
class FormalStepLoader(BaseDatasetLoader):
    ...
    def __init__(self, split: str = "train", max_items: Optional[int] = None):
        ...
        self.split = split
        self.dataset_name = "liuchengwu/FormalStep"

    def load(self) -> Iterator[DatasetItem]:
        """Load FormalStep dataset items."""
        if not DATASETS_AVAILABLE:
            raise RuntimeError("datasets package not available. Install with: pip install datasets")

        try:
            logger.info(f"Loading FormalStep dataset: {self.dataset_name}")
            ...
            for idx, item in enumerate(dataset):
                ...
                solution = self._process_solution(item.get("previous_steps", ""))
            ...
            logger.error(f"Error loading FormalStep dataset: {e}")
            raise
    ...
    def get_info(self) -> Dict[str, Any]:
        """Get information about the FormalStep dataset."""
        return {
            "name": "FormalStep",
